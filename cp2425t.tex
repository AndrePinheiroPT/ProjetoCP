...
\begin{eqnarray*}
\start
\ensuremath{\begin{lcbr}\Varid{g}\;[\mskip1.5mu \mskip1.5mu]\mathrel{=}[\mskip1.5mu \mskip1.5mu]\\\Varid{g}\;\Varid{l}\mathrel{=}\mathbf{in}\comp (\Varid{id}\times\Varid{g})\comp \Varid{f}\;\Varid{l}\end{lcbr}}
\end{eqnarray*}
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{geq}\mathrel{=}\uncurry{(\geq )}{}\<[E]%
\\
\>[B]{}\Varid{len}\mathrel{=}\Varid{fromIntegral}\comp \length {}\<[E]%
\\
\>[B]{}\Varid{f}\mathrel{=}\conj{\Varid{mul}\comp (\Varid{id}\times\Varid{len})}{\p2}\comp (\Varid{head}\times\Varid{tail})\comp \Varid{diag}\comp (\mcond{\Varid{geq}\comp (\Varid{head}\times\Varid{last})\comp \Varid{diag}}{\Varid{reverse}}{\Varid{id}}){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\ensuremath{\begin{lcbr}\Varid{g}\comp \Varid{nil}\mathrel{=}\mathbf{in}\;\\\Varid{g}\comp \Varid{id}\mathrel{=}\mathbf{in}\comp (\Varid{id}\times\Varid{g})\comp \Varid{f}\end{lcbr}}
\just\equiv{ definição de in, Eq-+ }
\ensuremath{\Varid{g}\comp \mathsf{in}_{id}\mathrel{=}\alt{\;\mathbf{in}\;}{\;\mathbf{in}\comp (\Varid{id}\times\Varid{g})\comp \Varid{f}}}
\just\equiv{ (33), Fusão-+ }
\ensuremath{\Varid{g}\mathrel{=}\mathbf{in}\comp \alt{\Varid{id}}{(\Varid{id}\times\Varid{g})\comp \Varid{f}}\comp \mathsf{out}_{id}}
\just\equiv{ Absorção-+ }
\ensuremath{\Varid{g}\mathrel{=}\mathbf{in}\comp \alt{\Varid{id}}{\Varid{id}\times\Varid{g}}\comp (\Varid{id}\mathbin{+}\Varid{f})\comp \mathsf{out}_{id}}
\just\equiv{ Universal-ana }
\ensuremath{\Varid{g}\mathrel{=}\ana{(\Varid{id}\mathbin{+}\Varid{f})\comp \mathsf{out}_{id}}}
\qed
\end{eqnarray*}
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{h}\mathrel{=}\llparenthesis\, \alt{\Varid{zero}}{\Varid{umax}}\,\rrparenthesis{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{mostwater}\mathrel{=}\llbracket\, \alt{\Varid{zero}}{\Varid{umax}},\,(\Varid{id}+\Varid{f})\comp \mathsf{out}_{id}\,\rrbracket{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
onde:
\begin{hscode}\SaveRestoreHook
...
\begin{eqnarray*}
\xymatrix{
    \ensuremath{[\mskip1.5mu \N_0\mskip1.5mu]}
           \ar[d]_-{\ensuremath{\ana{\Varid{g}}}}
           \ar[r]_-{\ensuremath{\mathsf{out}_{id}}}
&
    \ensuremath{\mathrm{1}\mathbin{+}[\mskip1.5mu \N_0\mskip1.5mu]}
           \ar[r]_-{\ensuremath{\Varid{id}\mathbin{+}\Varid{f}}}
& 
	\ensuremath{\mathrm{1}\mathbin{+}\N_0\times[\mskip1.5mu \N_0\mskip1.5mu]}
           \ar[d]^{\ensuremath{\Varid{id}\mathbin{+}\Varid{id}\times\ana{\Varid{g}}}}
\\
     \ensuremath{[\mskip1.5mu \N_0\mskip1.5mu]}
           \ar[d]_-{\ensuremath{\llparenthesis\, \Varid{h}\,\rrparenthesis}}
& &
     \ensuremath{\mathrm{1}\mathbin{+}\N_0\times[\mskip1.5mu \N_0\mskip1.5mu]}
           \ar[ll]^-{\ensuremath{\mathbf{in}}}
           \ar[d]^-{\ensuremath{\Varid{id}\mathbin{+}\Varid{id}\times\Varid{h}}}
\\
     \ensuremath{\N_0}
& &
     \ensuremath{\mathrm{1}\mathbin{+}\N_0\times\N_0}
           \ar[ll]^-{\ensuremath{\alt{\Varid{zero}}{\Varid{umax}}}}
}
\end{eqnarray*}
